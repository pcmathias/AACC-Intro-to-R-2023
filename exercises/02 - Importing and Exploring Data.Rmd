...
Take a look at the *Files* pane on the bottom right. Note that there is a file named *biochem_wide.csv*. The objective of this exercise is to *import* that CSV file into R as a data frame, and then take a first look at its contents using the *Data Viewer*.
...
In the following code chunk, we first load the *tidyverse* package and then use the *read_csv()* function to load the *biochem_wide.csv* file and capture it in an object named *biochem*. 
...
On the far right next to *biochem*, you will spot a symbol that looks like a small table. Click on it to make the *Data Viewer* appear.
