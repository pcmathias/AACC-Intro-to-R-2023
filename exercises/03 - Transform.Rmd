...
filter(biochem, last_name=="lopez")
```
...
cc_outliers <- biochem %>% 
  ...
  filter(total_tat > 0.5)    # filter only turnaround time greater than 10 hours
cc_outliers
```
...
How might you use arrange() to determine the highest and lowest Sodium result in the dataset? (Hint: Try filtering all `sodium_mmol_l` tests and then using arrange on the `value` column.) 
