...
A. You are wanting to confirm a reference interval for women over 50 for alkaline phophatase. Find every "alkaline_phosphatase" test for "Female" patients aged 50 and over. The column in which age is found is named `age`, gender is found in `gender` and test is in `test`. Assign the final new data frame to the object `biochem_3a`.

```{r Your Turn 3a, eval=FALSE}
#replace the ____ with a logical statement identifying tests for patients greater than 50
biochem_3a_1 <- filter(biochem, age ___ 50) 
biochem_3a_2 <- filter(biochem_3a_1, _______ "alkaline_phosphatase")
biochem_3a <- filter(biochem_3a_2, _________________)
biochem_3a
```
...
#replace the ____ with a logical statement identifying tests for patients with demo_group equal to "outpatient"
biochem_3b <- filter(__________, ________ __ ____________)
biochem_3b
```
...
biochem_3c <- filter(_______, is.na(________))
biochem_3c
```
...
mutate(biochem, c_r_tat_mins = col_rec_tat*__)
```

## Your Turn 3

Create a new column using the mutate() function that contains the total TAT in minutes. Hint: The total TAT is the sum of `col_rec_tat` and `rec_ver_tat`

```{r Your Turn 6, eval=FALSE}
mutate(biochem, total_tat = __*(___________ _ ___________)) 
```
...
tat_outliers_1 <- filter(biochem, pat_type == 'cancer center')
tat_outliers_2 <- mutate(tat_outliers_1, total_tat = col_rec_tat + rec_ver_tat)
tat_outliers <- filter(tat_outliers_2, total_tat > 2)

# vs

# This code accomplishes the same task with less code
tat_outliers_pipe <- biochem %>% 
	filter(pat_type == 'cancer center') %>% 
...
1.  Filters to tests from the patient type (`pat_type`) of "icu"

2.  Adds a column with the total turnaround time (sum of `col_rec_tat` and `rec_ver_tat`)

3.  Filters to include only the rows with a calculated turnaround time greater than 2 hour

4. Filter to include only rows that have a phosphorus result

Using "\<-", assign the result to a new variable, call it whatever you want.

```{r your_turn_4, eval=FALSE}
cc_outliers <- biochem %>% 
  ______(________ __ _____)%>% # filter to the pat_type with the name "icu"
  mutate(total_tat = col_rec_tat + rec_ver_tat) ___ # calculate total turnaround time
  filter(_________ _ _) %>%    # filter only turnaround time greater than 2 hours
  filter( ___ __ "phosphorus") # filter for phosphorus results
...
-   What is the typical TAT for the ICU?
-   Given this data, does it appear that there is a chronic problem in TAT for the ICU, or was this test an outlier?
...
select(biochem, MRN = mrn, age, last_name) # select columns and rename one
```

### Bonus Your Turn - select

A. Alter the code to select just the `payor_groups` column. Assign the result to `biochem_2a` and show the resulting data frame by executing the following line with just the name of the object: `biochem_2a`

```{r your_turn_select_a, eval=FALSE}
# Remove the "_______" and replace it with your answer
biochem_2a <- ______(_______, ____________)
biochem_2a
```
