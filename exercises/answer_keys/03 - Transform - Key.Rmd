...
```{r}
# Filters the biochem testing dataset to just rows in which the mrn column is equal to 121444
...
cc_outliers <- biochem %>% 
  filter(pat_type == "icu")%>% # filter to the pat_type with the name "icu"
  mutate(total_tat = col_rec_tat + rec_ver_tat) %>% # calculate total turnaround time
  filter(total_tat > 2) %>%    # filter only turnaround time greater than 2 hours
  filter( test == "phosphorus") # filter for phosphorus results
cc_outliers
```
...
biochem_2a <- select(biochem, payor_groups)
biochem_2a
```
...
tail(biochem_sodium)
## tail returns the last 6 results in the dataframe
```
